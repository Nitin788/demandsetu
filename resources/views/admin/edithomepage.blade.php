...
<div class="page-heading">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" style="margin-left: 30px;">
            Edit Home Page
        </li>
    </ol>
</div>
<div class="page-content fade-in-up">
    <div class="row">
        <div class="col-md-12">
            <div class="ibox">
                <div class="ibox-head">
                    <div class="ibox-title">Edit Home Page</div>
                ...
                <div class="ibox-body">
                    <form class="form-horizontal" action="{{ route('homepages.update', $homepages->id) }}" method="POST"
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div id="slider-container">
                                    @foreach($homepages->slider_images ?? [] as $index => $slider)
                                        <div class="form-group row slider-group">
                                            <label class="col-sm-2 col-form-label font-weight-bold">Slider Image</label>
                                            <div class="col-sm-4">
                                                <input class="form-control" name="slider_images[{{$index}}]" type="file">
                                                @if(isset($slider['image']) && $slider['image'])
                                                    <img src="{{ asset($slider['image']) }}" width="100px" height="50px"
                                                        alt="Slider Image" class="img-fluid mt-2">
                                                @endif
                                            </div>
                                            <label class="col-sm-2 col-form-label font-weight-bold">Title</label>
                                            <div class="col-sm-3">
                                                <input class="form-control" name="titles[{{$index}}]" type="text"
                                                    value="{{ old('titles.' . $index, $slider['title'] ?? '') }}"
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div id="banner-container">
                                    @foreach(is_array($homepages->offer_image) ? $homepages->offer_image : [$homepages->offer_image] as $index => $banner)
                                        <div class="form-group row banner-group">
                                            ...
                                            <div class="col-sm-4">
                                                <input class="form-control @error('session_banners.*') is-invalid @enderror"
                                                    name="session_banners[{{$index}}]" multiple type="file">
                                                @if ($banner)
                                                    <img src="{{ asset($banner) }}" width="100px" height="50px"
                                                        alt="Session Banner" class="img-fluid mt-2">
                                                @endif
                        ...
                        <div id="card-sections-container">
                            @foreach($homepages->offer_card as $sectionIndex => $section)
                                <div class="card mb-4 card-section" id="card-section-{{$sectionIndex}}">
                                    <div class="card-header">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-4">
                                                <input class="form-control" name="card_title[{{$sectionIndex}}]" type="text"
                                                    value="{{ old('card_title.' . $sectionIndex, $section['title']) }}"
                                                    placeholder="Enter card title">
                                            </div>
                                        </div>
                                        <!-- Card Container for Each Section -->
                                        <div id="card-container-{{$sectionIndex}}" class="card-container">
                                                ...
                                                <div class="form-group row card-group">
                                                    <div class="col-sm-3">
                                                        ...
                                                        <input class="form-control"
                                                            name="card_images[{{$sectionIndex}}][{{$cardIndex}}]" type="file">
                                                        @if ($card['image'])
                                                            <img src="{{ asset($card['image']) }}" width="100px" height="50px"
                                                                alt="Card Image" class="img-fluid mt-2">
                                                        @endif
                                                    </div>
                                                    <div class="col-sm-3">
                                                        <label class="font-weight-bold">Card Destination</label>
                                                        <input class="form-control"
                                                            name="card_destinations[{{$sectionIndex}}][{{$cardIndex}}]"
                                                            type="text"
                                                            value="{{ old('card_destinations.' . $sectionIndex . '.' . $cardIndex, $card['destination']) }}"
                                                            placeholder="Enter card destination">
                                                    </div>
                                                    <div class="col-sm-2">
                                                        <button type="button"
                                                            class="btn btn-danger btn-remove-card-row">X</button>
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
                        ...
                        <div class="row mb-3 ml-2">
                            <div class="btn-1 mr-3">
                                <button type="button" id="add-new-card-section" class="btn btn-success">+ Add New
                                    Section</button>
                            </div>
                        </div>
                        <!-- Submit Button -->
...
<script>
    ...
    document.addEventListener('click', function (event) {
        // Adding a new row inside the card section (for card image and destination)
        ...
        if (event.target && event.target.id === 'add-new-card-section') {
            ...
            const newCardSection = lastCardSection.cloneNode(true);
